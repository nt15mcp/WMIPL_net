...
$dummy_shooters = array();
$result = $conn->query("CALL current_dummies()");
while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
    $dummy_shooters += array($row['name'] => $row['week']);
}
        ...
        foreach ($numbers as $number => $shooters) {
            if (is_numeric($number)) {
                ...
                foreach ($shooters as $shooter => $scores) {
                    ...
                    $lyas += $scores['lya'];
                    if (array_key_exists($shooter, $dummy_shooters)) {
                        $divisions[$div][$team][$number] = array('DUMMY > WK' . $dummy_shooters[$shooter] => $divisions[$div][$team][$number][$shooter]);
                    }
                }
            }
        }
...
$individual_winners = array(
    'A' => array('high' => 0, 'avg' => 0, 'most_improv' => 0),
